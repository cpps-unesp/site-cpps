...
**Herança**
A herança é um princípio que permite criar uma nova classe (chamada de classe filha) a partir de uma classe já existente (chamada de classe mãe ou superclasse). Utilisamos as classs para reaproveitar códigos, já que a classe filha herda automaticamente todos os atributos e métodos da classe mãe, além de poder adicionar novas características exclusivas.  
**Polimorfismo** 
